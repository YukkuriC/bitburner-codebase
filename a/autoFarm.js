...
function runScript(ns, action, host, target) {
    ...
    if (nthread > 0) ns.exec(file, host, nthread, target || host)
...
function chooseAction(ns, host, target) {
    ...
    for (var script of FILES) {
        if (ns.scriptRunning(script, host)) return null
    }
...
export async function main(ns) {
    ...
    while (loop++) {
        ...
        if (newActionCount < 0.1 * servers.length) {
            // too many wait
            CONFIG.SLEEP_INTERVAL++
            CONFIG.SLEEP_INTERVAL *= CONFIG.SLEEP_INCREASE_RATE
        } else if (newActionCount > 0.9 * servers.length) {
            // too long
