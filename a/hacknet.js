...
export async function main(ns) {
    ...
    while (1) {
        ...
        if (nodeCount < maxCount) actions.push([newCost / calcFunc(), newCost, ns.hacknet.purchaseNode, 'Buy new NODE'])
        // upgrades
        for (var idx = 0; idx < nodeCount; idx++) analyzeNodeClosure(idx)
        // sort
    ...
    function analyzeNodeClosure(idx) {
        ...
        actions.push([costLvl / (calcFunc(curLevel + 1, curRam, curCores) - curProd), costLvl, () => ns.hacknet.upgradeLevel(idx), prefix + 'LEVEL'])
        actions.push([costRam / (calcFunc(curLevel, curRam * 2, curCores) - curProd), costRam, () => ns.hacknet.upgradeRam(idx), prefix + 'RAM'])
        actions.push([costCore / (calcFunc(curLevel, curRam, curCores + 1) - curProd), costCore, () => ns.hacknet.upgradeCore(idx), prefix + 'CORE'])
    }
}
