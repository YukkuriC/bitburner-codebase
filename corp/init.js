...
async function bootDivision(C, division) {
    try {
        C.expandIndustry(division, division)
    } catch (e) {}
    ...
    for (var city of Cities) {
        ...
        try {
            C.purchaseWarehouse(division, city)
        } catch (e) {}
    }
    ...
    for (var city of divisionData.cities) {
        ...
        while (ee.length < officeInfo.size) ee.push(await C.hireEmployee(division, city).name)
...
function setSupply(C, division) {
    ...
    for (var city of divisionData.cities) {
        ...
        for (var prod of SellMaterials[division]) C.sellMaterial(division, city, prod, 'MAX', 'MP')
    }
}
